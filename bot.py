...
token = "OTQ4OTY2MjcxOTM5MTI5Mzg1.YiDftg.JNu_XWdrU2ztBODw2T7EEyQYAUM"
...
async def sms(ctx, phone, amount:int):
    ...
    if (amount < 201):
      ...
      embes = discord.Embed(title="sᴘᴀᴍsᴍs", description=" 𝖇𝖔𝖙 𝖇𝖞 : 𝖅-𝕹𝖎𝖌𝖍𝖙𝕾𝖍𝖔𝖙 ",color=0xff4612)
      embes.add_field(name="กำลังยิงไปที่เบอร์",value=phone)
      ima = "https://i.gifer.com/3OJtw.gif"
      ...
      BBot(phone,amount)
    	...
    	await ctx.channel.send("กูบอกว่ายิงไม่เกิน200 ไอสัสพวกหน้าʜᴇᴇ ไปอ่านห้องวิธีใช้ด้วยอั่ยกั่ย")
...
@bot.command()
async def help(ctx):
	emBed = discord.Embed(title=" 𝖇𝖔𝖙 𝖇𝖞 :  𝖅-𝕹𝖎𝖌𝖍𝖙𝕾𝖍𝖔𝖙 ",description="ʜᴏᴡ",color=0xff4612)
	emBed.add_field(name="นี้วิธีใช้ดูซะอั่ยกั่ย(. ❛ ᴗ ❛.)",value="!sms [เบอร์] [จำนวน]")
